...
class _MainShellState extends State<MainShell> {
  ...
  /// Firestore'a periyodik ping atarak çevrimiçi/çevrimdışı durumu tespit eder.
  /// Harici connectivity paketi gerektirmez.
  void _startConnectivityMonitor() {
    final controller = StreamController<bool>.broadcast();
    _connectivitySub = controller.stream.listen((offline) {
      if (mounted && _isOffline != offline) {
        setState(() => _isOffline = offline);
      }
    });

    // Her 10 saniyede bir Firestore'a küçük bir okuma yap
    Timer.periodic(const Duration(seconds: 10), (timer) async {
      ...
      try {
        ...
        controller.add(false); // çevrimiçi
      } catch (_) {
        controller.add(true); // çevrimdışı
      }
    });
  }
...
/// Çevrimdışı durumda AppBar altında gösterilen ince banner.
