...
class _NewEventScreenState extends State<NewEventScreen> {
  ...
  Future<void> _submitEvent() async {
    ...
    if (_isEditMode) {
      // Düzenleme modunda: initialData'dan createdBy'ı al
      // (event_detail_screen zaten Firestore'dan okumuş ve geçirmiş)
      final createdBy =
          (widget.initialData?['createdBy'] as String?)?.trim() ?? '';

      // Eğer initialData'da yoksa Firestore'dan kontrol et
      if (createdBy.isEmpty) {
        final existingDoc = await FirebaseFirestore.instance
            ...
            .get();
        final dbCreatedBy =
            (existingDoc.data()?['createdBy'] as String?)?.trim() ?? '';
        if (currentUid.isEmpty ||
            (dbCreatedBy.isNotEmpty && dbCreatedBy != currentUid)) {
          if (!mounted) return;
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
      ...
      } else if (currentUid.isEmpty || createdBy != currentUid) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Bu etkinliği sadece oluşturan kişi düzenleyebilir.'),
            backgroundColor: Colors.red,
            duration: Duration(seconds: 4),
          ),
        );
      ...
      // Yeni ekleme modunda: admin veya kulüp başkanı olmalı
