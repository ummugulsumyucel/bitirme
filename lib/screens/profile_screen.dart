...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    String? id;
    try {
      // Önce Firebase Auth'tan direkt UID al — en güvenilir kaynak
      id = FirebaseAuth.instance.currentUser?.uid;
      if (id != null && id.isNotEmpty) {
        await SessionService.setUserDocId(id); // cache'i güncelle
      } else {
  ...
  Widget _buildProfileCard(Map<String, dynamic> userData) {
    final fullName = (userData['fullName'] as String?) ?? 'Kullanıcı';
    final role = (userData['role'] as String?)?.trim() ?? 'student';
