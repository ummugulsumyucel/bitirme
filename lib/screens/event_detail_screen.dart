...
import '../widgets/event_reminder_dialog.dart';
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          ...
          final liveLabel =
              (data['label'] as String?)?.trim() ?? widget.label ?? '';
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // ── Hatırlatıcı Butonu (herkes için) ──────────────────
                      _ReminderButton(
  ...
  Future<void> _onTap() async {
    ...
    try {
      ...
      if (_joined) {
        ...
        await Future.wait([attendeeRef.delete(), personalRef.delete()]);
...
/// Etkinlik banner resmi — hem Firebase Storage URL hem base64 data URL destekler.
class _EventImage extends StatelessWidget {
  ...
  Widget _placeholder() => const Center(
    child: Icon(Icons.broken_image_outlined, size: 60, color: Colors.white38),
  );
}

/// Hatırlatıcı butonu widget'ı
...
class _ReminderButtonState extends State<_ReminderButton> {
  ...
  Future<void> _checkReminder() async {
    try {
      final hasReminder = await EventReminderService().hasReminder(
        widget.eventId,
      );
      if (mounted) {
        setState(() {
  ...
  Future<void> _showReminderDialog() async {
    try {
      ...
      final result = await showEventReminderDialog(
        context: context,
        eventId: widget.eventId,
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: OutlinedButton.icon(
        ...
        label: Text(
          _hasReminder ? 'Hatırlatıcı Ayarlandı ✓' : 'Hatırlatıcı Ayarla',
          style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 15),
        ),
        style: OutlinedButton.styleFrom(
          padding: const EdgeInsets.symmetric(vertical: 14),
          side: BorderSide(
            color: _hasReminder ? Colors.green : scheme.primary,
            width: 2,
          ),
          foregroundColor: _hasReminder ? Colors.green : scheme.primary,
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(12),
          ),
        ),
